...
if ((player distance (markerPos "ent_spawn_kaelevra") > 70) || _etat == 1) exitWith {
...
if (life_eat) exitWith {["Info", "Vous êtes en train de manger !"] spawn life_fnc_showPredefinedNotification;};
life_eat = true;
...
switch (_item) do { 
	...
	case "platJour" : {
		...
		if ((independent countSide playableUnits) == 0) then {player setDamage 0;};
	};
