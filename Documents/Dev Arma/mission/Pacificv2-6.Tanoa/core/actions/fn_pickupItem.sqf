...
if((time - life_action_delay) < 2) exitWith {["Warning", localize "STR_NOTF_ActionDelay"] spawn life_fnc_showPredefinedNotification; INUSE(_this);};
if(isNull _this OR {player distance _this > 3}) exitWith {INUSE(_this);};

_itemInfo = _this GVAR ["item",[]]; if(EQUAL(count _itemInfo,0)) exitWith {	
...
if(_diff <= 0) exitWith {["Warning", localize "STR_NOTF_InvFull"] spawn life_fnc_showPredefinedNotification; INUSE(_this);};
