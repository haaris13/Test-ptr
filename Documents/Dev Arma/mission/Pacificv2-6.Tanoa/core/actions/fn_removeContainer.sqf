...
if(!(_house in life_vehicles)) exitWith {["Error", localize "STR_ISTR_Box_NotinHouse"] spawn life_fnc_showPredefinedNotification;};
if(isNull _container) exitWith {};
_containers = _house GVAR ["containers",[]];
closeDialog 0;

_action = [
	format[localize "STR_House_DeleteContainerMSG"],
		localize "STR_pInAct_RemoveContainer",
		localize "STR_Global_Remove",
...
if(_action) then {
	...
	if(EQUAL(_diff,0)) exitWith {["Warning", localize "STR_NOTF_InvFull"] spawn life_fnc_showPredefinedNotification;};
