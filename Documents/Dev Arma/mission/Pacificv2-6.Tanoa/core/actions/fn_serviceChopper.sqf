...
if(life_action_inUse) exitWith {["Error", localize "STR_NOTF_Action"] spawn life_fnc_showPredefinedNotification;};

_search = nearestObjects[getPos air_sp, ["Air"],5];
if(EQUAL(count _search,0)) exitWith {["Error", localize "STR_Service_Chopper_NoAir"] spawn life_fnc_showPredefinedNotification;};
if(CASH < 1000) exitWith {["Error", localize "STR_Serive_Chopper_NotEnough"] spawn life_fnc_showPredefinedNotification;};
...
if(!alive SEL(_search,0) || SEL(_search,0) distance air_sp > 10) exitWith {life_action_inUse = false; ["Error", localize "STR_Service_Chopper_Missing"] spawn life_fnc_showPredefinedNotification;};
