...
if (ITEM_VALUE("shovel") isEqualTo 0) exitWith {["Error", "Vous avez besoin d'une pelle !"] spawn life_fnc_showPredefinedNotification;};
if (_inWater isEqualTo 1 && ((getPosATL player) select 2) > 2) exitWith {["Warning", "Vous devez être près du sol !"] spawn life_fnc_showPredefinedNotification;};
