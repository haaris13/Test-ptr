...
if(([] call(life_adminlevel)) < 5) exitWith {closeDialog 0;};
...
if ((count life_admin_stopAction) > 0) exitWith {
	...
	["InfoWithSound", "Vous avez Désactivé un Stop Action sur cette Zone !"] spawn life_fnc_showPredefinedNotification;
...
stopActionZone = {
	...
	[_units] spawn {
		...
		_text = "<t color='#ff0000'>Attention ! L'action dans la zone où vous êtes vient d'être Stoppé par un Admin ! Il est maintenant interdit d'effectuer une action RP Ici !</t>";
	...
	openMap [false, false];
	["Accepted", format["Action Stoppé! Pour %1 Unités !", (count _units)]] spawn life_fnc_showPredefinedNotification;
