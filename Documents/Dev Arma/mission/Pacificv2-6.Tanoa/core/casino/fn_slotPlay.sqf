...
if(CASH < _price) exitWith {["Error", format["Vous n'avez pas assez pas d'argent, il vous faut (%1$)",[_price] call life_fnc_numberText]] spawn life_fnc_showPredefinedNotification;};
CASH = CASH - _price;
...
if (_bad) exitWith {closeDialog 0; ["Error", "Vous n'avez plus de tickets !"] spawn life_fnc_showPredefinedNotification;};
