...
if(!(_building isKindOf "House_F")) exitWith {["Error", "Vous n'êtes pas en train de regarder une porte d'une maison."] spawn life_fnc_showPredefinedNotification;};
["Error", "Vous n'êtes pas devant une porte d'une maison."] spawn life_fnc_showPredefinedNotification;
_doors = 1;
...
if(EQUAL(_door,0)) exitWith {["Error", localize "STR_Cop_NotaDoor"] spawn life_fnc_showPredefinedNotification;}; //Not near a door to be broken into.
_doorN = _building GVAR [format["bis_disabled_Door_%1",_door],0];
if(EQUAL(_doorN,1)) exitWith {["Error", localize "STR_House_FedDoor_Locked"] spawn life_fnc_showPredefinedNotification;};
