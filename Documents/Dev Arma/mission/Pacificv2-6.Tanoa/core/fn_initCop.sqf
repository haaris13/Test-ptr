...
if ((FETCH_CONST(life_coplevel) < 8) && player isEqualTo cop_1) then {
	["NotGeneral",false,true] call BIS_fnc_endMission;
	sleep 35;
};
...
[] spawn {
	while {true} do {
		...
		if ((player ammo (handgunWeapon player)) > 1) then {
...
//box_bank addAction["Fermer la banque",life_fnc_fixBank,"",0,false,false,"",'playerSide == west && player distance box_bank < 4 '];