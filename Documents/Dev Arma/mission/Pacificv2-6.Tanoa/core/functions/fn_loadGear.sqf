...
_gear = _itemArray select 0;

if (count _gear == 10) then {

    // Linkeds
    _linkeds = _gear select 9;
    _linkedsFix = [];

    {
        ...
        if (_item find "tf_radio_civ" >= 0) then {
            _linkedsFix pushBack "tf_radio_civ";
            _added = true;
        };

        if (_item find "tf_radio_reb" >= 0) then {
            _linkedsFix pushBack "tf_radio_reb";
            _added = true;
        };

        if (_item find "tf_radio_gen" >= 0) then {
            _linkedsFix pushBack "tf_radio_gen";
            _added = true;
        };

        if (_item find "tf_radio_med" >= 0) then {
            _linkedsFix pushBack "tf_radio_med";
            _added = true;
        };

        if (!_added) then {
            _linkedsFix pushBack _item;
        };
    } forEach _linkeds;

    _gear set[9, _linkedsFix];
};
