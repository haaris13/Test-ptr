...
if(EQUAL(count _houseCfg,0) && playerSide == civilian) exitWith {["Error", localize "STR_House_NotBuyable"] spawn life_fnc_showPredefinedNotification;};
...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget GVAR "house_owner"}) then {
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_Btn4 ctrlSetText "Intéragir avec la porte";
		_Btn4 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_openNearDoor; closeDialog 0;";
		_Btn4 ctrlShow true;
	};
};