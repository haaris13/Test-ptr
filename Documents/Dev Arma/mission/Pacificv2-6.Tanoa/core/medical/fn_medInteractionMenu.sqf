...
if (!alive _curTarget) exitWith {
	...
	if (!life_mode_medic) then {
		...
		_Btn4 ctrlSetText "Annuler";
		_Btn4 buttonSetAction "life_mode_medic = false; closeDialog 0; ";
...
if (!_targetRestrained) exitWith {
	...
	if (
			(player distance (getMarkerPos "spawn_medic_1") > 200) && 
			(player distance (getMarkerPos "spawn_medic_2") > 200) && 
			(player distance (getMarkerPos "spawn_medic_3") > 200) && 
			(player distance (getMarkerPos "spawn_medic_4") > 200)) then {
		_Btn4 ctrlEnable false;
	};


	_Btn5 ctrlSetText "Donner Papier";
	_Btn5 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_papierMenuGiveOpen;";
	...
	if ((FETCH_CONST(life_medicLevel)) >= 12) then {
			...
			_Btn7 buttonSetAction "[life_pInact_curTarget, 3, 'MEDIC'] spawn life_fnc_whiteListTeam;";
