...
if (!isNull _killer && {_killer != _unit}) then {
	...
	[getPlayerUID _killer, getPlayerUID _unit, _weapons, (position _unit), str (side _killer), str (playerSide)] remoteExecCall ["TON_fnc_insertDeathLogs",RSERV];
} else {
	["No Killer", getPlayerUID _unit, "No Killer", (position _unit), "No Killer", str (playerSide)] remoteExecCall ["TON_fnc_insertDeathLogs",RSERV];
};
...
_unit SVAR ["transporting",nil,TRUE]; //Why the fuck do I have this? Is it used?
...
if !((vehicle _killer) isEqualTo (vehicle _unit)) then {
	if (side _killer isEqualTo civilian) then {
		 if (faction _unit isEqualTo "BLU_F") then {
		 	[false,20] remoteExecCall ["life_fnc_changeRep",_killer]; 
		 } else {
