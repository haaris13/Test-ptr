...
if(!isClass (missionConfigFile >> "Professions" >> _prof)) exitWith {
	["no_prof"] call life_fnc_addXP;
};
_level = false; 
_profMaxLevel = 1000;
_bonus = 0;
...
_legal = getNumber(missionConfigFile >> "Professions" >> _prof >> "legal");

if (_legal == 1 && (["vodka"] call life_fnc_hasAtouts)) then {
	_amount = round(_amount * 1.3);
};

if (_legal == 0 && ["blunt1"] call life_fnc_hasAtouts) then {
    _amount = round(_amount * 1.3);
};
...
if (_level) then {
	...
	if (_profNewLevel >= 1000) then {
		switch (_prof) do { 
			case "prof_craft" : {["prof_craft_750"] spawn life_fnc_achievementLearn;}; 
			case "prof_narcos" : {["prof_narcos_750"] spawn life_fnc_achievementLearn;}; 
			case "prof_mining" : {["prof_mining_750"] spawn life_fnc_achievementLearn;};
			case "prof_treasure" : {["prof_treasure_750"] spawn life_fnc_achievementLearn;};
			case "prof_contrebande" : {["prof_contrebande_750"] spawn life_fnc_achievementLearn;};
			default {}; 
		};
	};
