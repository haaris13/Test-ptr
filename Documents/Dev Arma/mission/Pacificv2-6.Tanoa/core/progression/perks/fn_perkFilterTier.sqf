...
if (_filteredTier isEqualTo 0) exitWith {["Error", "Une erreur est survenue !"] spawn life_fnc_showPredefinedNotification;;};
_playerSide = switch(playerSide) do {case west:{"cop"}; case civilian:{"civ"}; case independent:{"med"};};
_activePerk = false;

//-- Available Tier thing
CONTROL(17000,17020) ctrlSetText format ["Compétences Disponibles - Tier %1",_filteredTier];

//--Controls to edit
_availablePerkList = CONTROL(17000,17018);
_activePerkLB = CONTROL(17000,17015);
//--Clear list / textbox
...
//--Add available perks, and check for currently active perk
{
	...
	if ((_side isEqualTo _playerSide) && (_tier isEqualTo _filteredTier)) then {
		...
		if (((configName _x) in life_activePerks) && (!_activePerk)) then {
			...
			CONTROL(17000,17016) ctrlSetStructuredText parseText format ["%1",_description];
			//-- Add to active box
...
//-- Select a perk text
CONTROL(17000,17011) ctrlSetText format ["Selection"];
CONTROL(17000,17012) ctrlSetStructuredText parseText format["Sélectionnez une compétence dans la liste sur la gauche pour afficher ses bonus."];
...
if (_activePerk) then {
	...
	CONTROL(17000,17013) ctrlSetStructuredText parseText format ["<t color ='#ea0000' size = '0.8'>AVIS: Vous avez déjà une compétence sur ce palier.</t>"];
	CONTROL(17000,17017) ctrlSetStructuredText parseText format ["<t size = '0.8'>Coût: %1XP</t>", [(_price)] call life_fnc_numberText];

} else {

	CONTROL(17000,17030) ctrlEnable true; 
	CONTROL(17000,17031) ctrlEnable false;
	CONTROL(17000,17016) ctrlSetStructuredText parseText format [""];
	CONTROL(17000,17013) ctrlSetStructuredText parseText format [""];
	CONTROL(17000,17017) ctrlSetStructuredText parseText format [""];
};