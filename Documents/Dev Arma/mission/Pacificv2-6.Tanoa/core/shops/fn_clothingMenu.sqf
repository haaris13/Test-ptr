...
if(!(EQUAL(_license,""))) then {
	...
	if(!(LICENSE_VALUE(_license,_flag))) exitWith {["Error", localize "STR_Shop_Veh_NoLicense"] spawn life_fnc_showPredefinedNotification; _exit = true;};
};
...
if (_var == "sud" && ((group player) getVariable ["clan_id", -1]) != 90) exitWith {
	["Error", "Vous ne faites pas Parti du Sud !"] spawn life_fnc_showPredefinedNotification;
};
...
if((SEL(_this,3) in ["bruce","dive","reb","kart"] && playerSide != civilian)) exitWith {["Error", localize "STR_Shop_NotaCiv"] spawn life_fnc_showPredefinedNotification; closeDialog 0;};
//if((SEL(_this,3) isEqualTo "ins" && !license_civ_insurgent)) exitWith {["Error", "You are not allowed to access this shop"] spawn life_fnc_showPredefinedNotification;; closeDialog 0;};
if((SEL(_this,3) in ["cop"] && playerSide != west)) exitWith {["Error", localize "STR_Shop_NotaCop"] spawn life_fnc_showPredefinedNotification; closeDialog 0;};
if((SEL(_this,3) in ["dive"] && !license_civ_dive)) exitWith {["Error", localize "STR_Shop_NotaDive"] spawn life_fnc_showPredefinedNotification; closeDialog 0;};

if((SEL(_this,3) in ["onu"] && playerSide != civilian)) exitWith {["Error", "Vous ne faites pas parti de l'onu !"] spawn life_fnc_showPredefinedNotification;; closeDialog 0;};
if((SEL(_this,3) in ["onu"] && (call life_euLevel) <= 0)) exitWith {["Error", "Vous ne faites pas parti de l'onu !"] spawn life_fnc_showPredefinedNotification;; closeDialog 0;};
...
_cfgClothe = (missionConfigFile >> "Clothing" >> _var);
life_clothing_store_payment_type = getText(_cfgClothe >> "paymentType");
