...
if(dialog) exitWith {};
if(_shop == "") exitWith {};
if ((_shop == "civ_depan" || _shop == "narvalo_air") && ((call life_depanLevel) <= 0)) exitWith {};
if (_shop == "onu_car" && ((call life_euLevel) <= 0)) exitWith {};
if (_shop == "security_car" && !('security' in life_entreprises)) exitWith {};
if (_shop in ["reb_car", "reb_truck", "reb_air", "reb_ship"] && !license_civ_insurge) exitWith {};
if (_shop in ["mafia_car"] && !license_civ_mafia) exitWith {};
if (_shop in ["admin_car"] && !(getPlayerUID player in ["76561198059757685", "76561198019856443"])) exitWith {};
if (_shop in ["admin_car", "brinks_car"] && (FETCH_CONST(life_adminlevel) < 1)) exitWith {};


if (_shop == "president_car" && (gouv_president_uid != (getPlayerUID player))) exitWith {};
if(_sideCheck != sideUnknown && {playerSide != _sideCheck}) exitWith {["Error", localize "STR_Shop_Veh_NotAllowed"] spawn life_fnc_showPredefinedNotification;};
if(!createDialog "Life_Vehicle_Shop_v2") exitWith {};

["Shop","Vehicule",false] spawn life_fnc_showHelp;

_configShop = (missionConfigFile >> "CfgCarShops" >> _shop);

_paymentType = getText(_configShop >> "paymentType");
_isLegal = getNumber(_configShop >> "legal");
life_veh_store_payment_type = _paymentType;

if (life_veh_store_payment_type == "") then {
	life_veh_store_payment_type = "cash";
};

life_veh_store_is_legal = _isLegal;
