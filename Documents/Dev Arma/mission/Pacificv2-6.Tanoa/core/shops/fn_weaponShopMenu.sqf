...
if (_shopCfg isEqualTo "gun" && life_playerlevel < 5) exitWith {
	closeDialog 0;
	["Error", "Vous n'êtes pas niveau 5 !"] spawn life_fnc_showPredefinedNotification;
};
...
_cfgShop = (missionConfigFile >> "WeaponShops" >> _shopCfg);

life_weapon_shop_payment_type = getText(_cfgShop >> "paymentType");

if (life_weapon_shop_payment_type == "") then {
	life_weapon_shop_payment_type = "cash";
};
...
if(!(EQUAL(_flag,_shopSide))) exitWith {["Error", "Vous ne pouvez pas accéder à cette boutique."] spawn life_fnc_showPredefinedNotification;};
...
if (_shopCfg in ["genstore","civ_mechanic", "f_station_store"]) then {
