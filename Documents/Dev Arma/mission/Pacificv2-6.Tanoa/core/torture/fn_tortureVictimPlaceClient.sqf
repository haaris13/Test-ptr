...
while {player getVariable ["torture", false]} do {
	if(vehicle player == player) then {
		player playMove "Acts_ExecutionVictim_Loop";
	};
	
	_state = vehicle player;
	waitUntil {!(animationState player in _anims) || !(player getVariable ["torture", false]) || vehicle player != _state};
...
[_execPlayer] spawn life_fnc_restrain2;