...
_timer = ((_fueltoput * .015)/100); // 0.025 => (resultat) 0.125
if (_car distance player > 15 && vehicle player != player) exitwith {
	["Error", localize "STR_Distance_Vehicle_Pump"] spawn life_fnc_showPredefinedNotification;
	vehiclefuelList = [];
	life_action_inUse = false;
	closedialog 0;
};
...
if ((life_atmbank - (_fueltoput * (life_fuelPrices select 0)))> 0) then {
	...
	if (_car distance player > 15 || vehicle player != player || (life_fuel_station getVariable["citerne", 0] < _newSetFuel)) then {
		...
		if ((life_fuel_station getVariable["citerne", 0] < _newSetFuel)) then {
			["Error", "Il n'y a plus assez d'essence dans la Station !"] spawn life_fnc_showPredefinedNotification;
		} else {
			["Error", localize "STR_Distance_Vehicle_Pump"] spawn life_fnc_showPredefinedNotification;
		};
	...
	["Error", localize "STR_NOTF_NotEnoughMoney"] spawn life_fnc_showPredefinedNotification;
};
