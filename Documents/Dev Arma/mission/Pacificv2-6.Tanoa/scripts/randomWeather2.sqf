...
weatherTemplates = [
        ...
        ["Light Fog",[0,2,5,6],[0.4,0,[0.2,0.01,2.5],5,0]],
        ["Medium Fog",[5,6,7],[0.4,0,[0.4,0.005,5],0,3]],
        ["Dense Fog",[6],[0.5,0,[0.4,0.0025,7.5],1,2]]
	];
...
mb_fnc_InitialWeather = {
    ...
    _weatherInitialWindNS = _weatherInitialSettings select 4;	
		...
		if (rw2Debug == 1) then {[format ["Debug Initialized Weather - %1\nOvercast: %2\nRain/Snow: %3\nFog: %4\nWind EW|NS: %5|%6",weatherCurrentName,_weatherInitialOvercast,_weatherInitialRainSnow,_weatherInitialFog,_weatherInitialWindEW,_weatherInitialWindNS]] remoteExecCall ["life_fnc_dynamicText",-2];};
		diag_log format ["Debug Initialized Weather - %1\nOvercast: %2\nRain/Snow: %3\nFog: %4\nWind EW|NS: %5|%6",weatherCurrentName,_weatherInitialOvercast,_weatherInitialRainSnow,_weatherInitialFog,_weatherInitialWindEW,_weatherInitialWindNS];
};
...
mb_fnc_UpdateWeather = {
	...
	if (rw2Debug == 1) then {[format ["Debug Updating Weather - %1\nOvercast: %2\nRain/Snow: %3\nFog: %4\nWind EW/NS: %5|%6",weatherNextName,_weatherNextOvercast,_weatherNextRainSnow,_weatherNextFog,_weatherNextWindEW,_weatherNextWindNS]] remoteExecCall ["life_fnc_dynamicText",-2];};
	diag_log format ["Debug Updating Weather - %1\nOvercast: %2\nRain/Snow: %3\nFog: %4\nWind EW/NS: %5|%6",weatherNextName,_weatherNextOvercast,_weatherNextRainSnow,_weatherNextFog,_weatherNextWindEW,_weatherNextWindNS];
};
...
// Check if there is no ParamsArray, and pick random if so, otherwise pick from paramsArray.
      if(isNil('paramsArray') || (count paramsArray)  == 0) then {
