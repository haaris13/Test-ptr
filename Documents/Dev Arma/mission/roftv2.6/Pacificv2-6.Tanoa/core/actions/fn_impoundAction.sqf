...
if(EQUAL((count _vehicleData),0)) exitWith {
	[_vehicle] spawn life_fnc_deleteVehicle;
}; //Bad vehicle.

if ((_vehicle getVariable ["side", civilian]) isEqualTo west) exitWith {
	["Error", "Véhicule gendarmerie, impossible !"] spawn life_fnc_showPredefinedNotification;
};
...
if (["vehCopV1"] call life_fnc_hasPerk) then {
	_cpRate = _cpRate + 0.0010;
};

if (["vehCopV2"] call life_fnc_hasPerk) then {
	_cpRate = _cpRate + 0.0010;
};
...
if(player distance _vehicle > 10) exitWith {["Warning", localize "STR_NOTF_ImpoundingCancelled"] spawn life_fnc_showPredefinedNotification; life_action_inUse = false;};
...
if(EQUAL(count crew _vehicle,0)) then {
	...
	["Accepted", format[localize "STR_NOTF_Impounded",_type,_price]] spawn life_fnc_showPredefinedNotification;
	[0,"STR_NOTF_HasImpounded",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];

	ADD(BANK,_price);
	["impound"] call life_fnc_addXP;

} else {
	["Warning", localize "STR_NOTF_ImpoundingCancelled"] spawn life_fnc_showPredefinedNotification;
};
