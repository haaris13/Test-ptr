...
if(_reward > 0) then
{
	...
	{
		case 1: {
			_extra = "en obtenant tous les symboles égaux";
...

			{
				...
				if ((isPlayer _unit) && (player distance _unit) <= 30) then {
					_units pushBack _unit;
				};
			} forEach playableUnits;

			[player,"slot_jackpot"] remoteExecCall ["life_fnc_playSound3D",_units];
		};

		case 2: { _extra = "en obtenant 2 symboles égaux"; };
	};

	// Print a hint telling us what the win amount was
	["Accepted", format["Vous avez gagné %1$ %2.",[_reward] call life_fnc_numberText,_extra]] spawn life_fnc_showPredefinedNotification;
};