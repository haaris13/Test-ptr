...
if (_targetDown) exitWith {
	[life_pInact_curTarget] spawn life_fnc_tortureVictimUnPlace;
	closeDialog 0;
};
...
if (!_targetRestrained) exitWith {
	...
	if (_surrender) exitWith {
		...
		//Set Ticket Button
		_Btn2 ctrlSetText localize "STR_pInAct_TicketBtn";
		_Btn2 buttonSetAction "[life_pInact_curTarget] call life_fnc_ticketAction;";

		_Btn3 ctrlSetText "Donner Objets";
		_Btn3 buttonSetAction "closeDialog 0; ['load',life_pInact_curTarget] call life_fnc_giveItem;";

		_Btn4 ctrlSetText "Donner Argent";
		_Btn4 buttonSetAction "closeDialog 0; ['load',life_pInact_curTarget] call life_fnc_giveMoney;";

		_Btn5 ctrlSetText "Controler";
		_Btn5 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_fouille;";

		if ((ITEM_VALUE("mouchard") >= 1)) then {_Btn7 ctrlEnable true} else {_Btn7 ctrlEnable false};

		_Btn6 ctrlSetText "Placer Mouchard";
		_Btn6 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_placeMouchard;";
	...
	if (!_surrender) exitWith {
		...
		_Btn6 ctrlSetText "Signaler";
		_Btn6 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_reportPlayer;";
		
		if ((FETCH_CONST(life_coplevel)) >= 10) then {
			_Btn7 ctrlSetText "WhiteList";
			_Btn7 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_whiteListCop;";

			if ((player distance (getMarkerPos "cop_base") > 200)) then {
...
if (_targetRestrained) exitWith {
	...
	*/
	...
	_Btn4 ctrlSetText "Saisir les armes";
	_Btn4 buttonSetAction "[life_pInact_curTarget] call life_fnc_seizeWeapons;";
	...
	_Btn6 ctrlSetText "Retirer Communication";
	_Btn6 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_retirerCom;";

	_Btn7 ctrlSetText "Arracher les clefs";
	_Btn7 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_arracherClef;";
