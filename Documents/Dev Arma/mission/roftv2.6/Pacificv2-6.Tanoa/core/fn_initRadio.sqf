...
if (isNil "TFAR_fnc_isTeamSpeakPluginEnabled") exitWith {
	...
	while {true} do {
		...
		uiSleep 5;
		["Abonnez vous à Pacific Island65 sur le SteamWorkshop pour pouvoir l'avoir (Uniquement )!"] spawn life_fnc_dynamicText;
	};
};

tf_radio_channel_name = "tfr26";  // Nom du Channel TeamSpeak  
tf_radio_channel_password = "123456";  // Mot de passe du channel TeamSpeak  
TF_terrain_interception_coefficient = 0;  // Réglage de l'interception sur le terrain  
...
[] spawn
{
	while {true} do {
		_TFenabled = [] call TFAR_fnc_isTeamSpeakPluginEnabled;
		_onTsServer = "Pay Or Die " == (call TFAR_fnc_getTeamSpeakServerName);
		_onChannel = "TaskForceLife" == (call TFAR_fnc_getTeamSpeakChannelName);
