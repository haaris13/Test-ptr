...
if (life_medic_canRevive < 0) exitWith {["Error", "Un Check Up est nécésaire avant un massage cardiaque !"] spawn life_fnc_showPredefinedNotification;};
if (_target getVariable ["executed", false]) exitWith {["Error", "Cette personne est décédée."] spawn life_fnc_showPredefinedNotification;};
...
if(_target GVAR ["Revive",FALSE]) exitWith {["Error", localize "STR_Medic_RevivedRespawned"] spawn life_fnc_showPredefinedNotification;};
