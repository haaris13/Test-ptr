...
["Error", localize "STR_House_Door_NotNear"] spawn life_fnc_showPredefinedNotification;
if (!isNull _house && _house isKindOf "House_F" && {player distance _house < 12}) then{
	if(([player] call life_fnc_PlayerInBuilding)) then {
		if(!(_house in life_vehicles) OR isNil {_house GVAR "house_owner"}) then {
			...
			["Error", localize "STR_House_Container_House_Near_Owner"] spawn life_fnc_showPredefinedNotification;
		} else {
		...
		[_container] spawn life_fnc_deleteVehicle;
		[true,_type,_number] call life_fnc_handleInv;
		["Error", localize "STR_House_Container_House_Near"] spawn life_fnc_showPredefinedNotification;
	};
} else {
	[_container] spawn life_fnc_deleteVehicle;
	[true,_type,_number] call life_fnc_handleInv;
