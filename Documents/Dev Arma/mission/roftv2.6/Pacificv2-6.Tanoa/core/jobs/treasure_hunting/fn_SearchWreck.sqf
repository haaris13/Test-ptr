...
if ((getPlayerUID player) in _players) exitWith {["Error", "C'est Vide !"] spawn life_fnc_showPredefinedNotification; life_action_inUse = false; _wreck setVariable ["searched",true,false];};

if (count _wreckLoot isEqualTo 0) exitWith {["Error", "C'est Vide !"] spawn life_fnc_showPredefinedNotification; life_action_inUse = false; _wreck setVariable ["searched",true,false];};

if (dialog) exitWith {life_action_inUse = false;};
if (!createDialog "wreckSearchDialog") exitWith {["Error", "Il y a eu une erreur..."] spawn life_fnc_showPredefinedNotification; life_action_inUse = false;};
