...
if ((missionNamespace getVariable ["prof_treasure",0]) < _lvl) exitWith {
	["Error", "Votre niveau de Chasse aux trésors est pas assez élevé pour ouvrir ce coffre."] spawn life_fnc_showPredefinedNotification;
};
...
if (_combinedWeight > (life_maxWeight - life_carryWeight)) then { 
	["Error", "Vous n'avez pas assez de place pour ouvrir cette caisse !"] spawn life_fnc_showPredefinedNotification;
} else {
