...
if (isNil "bank_nb_breaker") exitWith {["Error", "Vous n'êtes pas le poseur de bombe !"] spawn life_fnc_showPredefinedNotification;};
...
if (count (_items) <= 0) exitWith {["Error", "Il n'y a pas d'argent dans le coffre !"] spawn life_fnc_showPredefinedNotification;};
