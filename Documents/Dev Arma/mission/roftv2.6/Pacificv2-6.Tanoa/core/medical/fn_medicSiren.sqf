#include "..\..\script_macros.hpp"
/*
	File: fn_medicSiren.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Starts the medic siren sound for other players
*/
private["_vehicle"];
_vehicle = param [0,ObjNull,[ObjNull]];

if(isNull _vehicle) exitWith {};
if(isNil {_vehicle GVAR "siren"}) exitWith {};

_vehicle say3D ["pompierSirenStart", 500, 1, true];
uiSleep 5.1071;

while {true} do {
	if(!(_vehicle GVAR "siren")) exitWith {};
	if(EQUAL(count crew _vehicle,0)) then {_vehicle SVAR ["siren",false,true]};
	if(!alive _vehicle) exitWith {};
	if(isNull _vehicle) exitWith {};
	_vehicle say3D ["pompierSiren", 500, 1, true];//Class/name specified in description.ext
	sleep 2.162;//Exactly matches the length of the audio file.
	if(!(_vehicle GVAR "siren")) exitWith {};
};