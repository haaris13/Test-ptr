...
if (_checked == "") exitWith {["Error", "Aucune Fiole n'est séléctionné !"] spawn life_fnc_showPredefinedNotification;};
if((lbCurSel Listleft) == -1) exitWith {["Error", "Aucun Ingredient n'est selectionné !"] spawn life_fnc_showPredefinedNotification;};
...
if(_num < 1) exitWith {["Error", "1 minimum !"] spawn life_fnc_showPredefinedNotification;};
...
if (_selValue < _num) exitWith {["Error", "Vous n'avez pas autant de quantité !"] spawn life_fnc_showPredefinedNotification;};

if ((_fioleTransf select 0) >= 100) exitWith {["Error", "La fiole séléctionné est pleine !"] spawn life_fnc_showPredefinedNotification;};

if ((_num + (_fioleTransf select 0)) > 100) exitWith {["Error", "La fiole va déborder avec cette quantité !"] spawn life_fnc_showPredefinedNotification;};
...
if (!_good) then {
	...
	if (_index != -1) then {
		...
		["Error", "bad !"] spawn life_fnc_showPredefinedNotification;;
	};
};
