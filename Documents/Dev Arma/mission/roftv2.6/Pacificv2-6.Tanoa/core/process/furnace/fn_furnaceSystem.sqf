...
_illegal = getNumber(_cfgFurnace >> _cfgSystem >> "illegal");
...
if (_illegal == 0 && ["processLegalV1"] call life_fnc_hasPerk) then {
    _itemRequiredTime = _itemRequiredTime - (_itemRequiredTime * 0.1);
};
...
_nbItems = (life_furnace_config select 1);
...
while {life_furnace_on && (!(life_furnace_config isEqualTo [])) && ((player distance life_furnacePnj) < 100)} do {
	...
	if (_itemRequiredTemp > life_furnace_temp) then {
		...
		if (_info >= 30) then {
			_info = 0;
			["InfoWithSound", "Le Minerai n'a pas l'air de fondre il semblerait que la température du four soit trop basse !"] spawn life_fnc_showPredefinedNotification;
		};
	};
	...
	if (_needTime <= time && !_give && ((_itemRequiredTemp == life_furnace_temp)) && !_giveBad) then {
		...
		_level = [_cfgSystem, _nbItems] call life_fnc_getLevelByFurnace;
