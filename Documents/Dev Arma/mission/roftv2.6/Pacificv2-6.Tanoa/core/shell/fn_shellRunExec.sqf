...
if (!([_file] call life_fnc_shellRunCan)) exitWith {
	[format["Le fichier %1 ne peut pas être executé !"], false] spawn life_fnc_shellPrint;
	[format["Il est actuelement vide, vous n'êtes pas assez compétent pour le coder !"], false] spawn life_fnc_shellPrint;
};
