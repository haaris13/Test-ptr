...
*/
diag_log "fn_vehicleShopLBChange";
...
diag_log format["fn_vehicleShopLBChange: %1", _className];
...
if (life_veh_store_is_legal == 1) then {
	_basePrice = round(["sop_vehicle", _basePrice] call life_fnc_gouvTaxesCalc);
};
...
_hasDlc = ([_className] call life_fnc_hasDlc);

_dlcText = "";

if (!_hasDlc) then {
	_dlcText = "<t color='#F42E2E'>DLC nécessaire</t>";
};
...
switch (true) do {
	case (!_hasDlc) : { ctrlEnable [2309, false];};
