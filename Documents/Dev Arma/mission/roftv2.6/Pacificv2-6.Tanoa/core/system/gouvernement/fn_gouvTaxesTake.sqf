/*
	File: fn_gouvTaxesTake.sqf

	Autor: IronBlade

	_price = (["illegal_vente", _price] call life_fnc_gouvTaxesPay);
*/
private ["_varPay", "_valuePay", "_curTaxes", "_valToPay", "_valToGvn", "_varT", "_valueT"];
_varPay = [_this,0,"",[""]] call BIS_fnc_param;
_valuePay = [_this,1,0,[0]] call BIS_fnc_param;

if (!isClass(missionConfigFile >> "CfgGouv" >> "Taxe" >> _varPay)) exitWith {_valuePay;};
