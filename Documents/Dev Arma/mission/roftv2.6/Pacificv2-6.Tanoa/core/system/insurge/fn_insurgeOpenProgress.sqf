/*
	File: fn_insurgeOpenProgress.sqf
...
createDialog "Insurge_Cash_Progress";

_display = findDisplay 2090;
_group = _display displayCtrl 2091;

_cfgInsurge = (missionConfigFile >> "CfgInsurge");
...
_lastAmount = 0;
{
	...
	_stat = abs(life_insurge_cash_value - _lastAmount);

	if (_stat > _gap) then {
		_stat = _gap;
	};

	if (_lastAmount > life_insurge_cash_value) then {
	...
	_controlButton buttonSetAction format["['%1'] spawn life_fnc_insurgeOpenProgressInfo;", _stepName];
