...
if((_used + _itemWeight) > _totalWeight) exitWith {["Error", "Il y a plus de place !"] spawn life_fnc_showPredefinedNotification;};
if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {["Error", "Vous n'avez pas assez d'items !"] spawn life_fnc_showPredefinedNotification;};
