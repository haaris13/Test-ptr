...
if (fuel life_demanteler_veh <= 0) exitWith {["Error", "Pas assez de carburant dans le véhicule de Démantélement !"] spawn life_fnc_showPredefinedNotification;};
if (life_demanteler_veh getVariable ["dem_inUse", false]) exitWith {["Error", "Véhicule déja utilisé !"] spawn life_fnc_showPredefinedNotification;};
...
if((player GVAR ["restrained",false])) exitWith {};
if(vehicle player != player) exitWith {};
if(player distance _vehicle > 30) exitWith {["Error", "Vous êtes trop loin !"] spawn life_fnc_showPredefinedNotification;};
if(life_demanteler_veh distance _vehicle > 15) exitWith {};
if(isEngineOn life_demanteler_veh) exitWith {["Error", "Démantélement annulé ! Le moteur est peut être allumé ?"] spawn life_fnc_showPredefinedNotification;};
if(fuel life_demanteler_veh == 0) exitWith {["Error", "Pas assez de carburant dans le véhicule de Démantélement !"] spawn life_fnc_showPredefinedNotification;};

_caches = (_vehicle getVariable ["trunk_caches", []]);

if (count _caches == 0) exitWith {["Error", "Aucune Contrebande !"] spawn life_fnc_showPredefinedNotification;};
