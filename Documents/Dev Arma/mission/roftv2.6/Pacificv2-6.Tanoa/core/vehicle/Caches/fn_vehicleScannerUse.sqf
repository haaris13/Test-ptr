...
if(!_isVehicle && !_player) exitWith {closeDialog 0; ["Error", "Aucun véhicule à proximité !"] spawn life_fnc_showPredefinedNotification;};

if((time - life_action_scanner) < (60 * 2)) exitWith {["Error", "Le Scanner est en cours de Recharge !"] spawn life_fnc_showPredefinedNotification;};
life_action_scanner = time;

if ((time - (_veh getVariable["time_scaner",(time - (70 * 2))])) < (60 * 2)) exitWith {["Error", "Le Scanner est en cours de Recharge !"] spawn life_fnc_showPredefinedNotification;};
...
if (_veh == player) exitWith {
	...
	if (life_clientMouchard) then {
		["Warning", "Un Mouchard a été détécté !"] spawn life_fnc_showPredefinedNotification;
...
if (count _caches == 0) exitWith {["InfoWithSound", "Rien d'anormal à signaler !"] spawn life_fnc_showPredefinedNotification;};
...
if (_detect) then {
	["Warning", "Quelque chose semble suspect..."] spawn life_fnc_showPredefinedNotification;
} else {
	["InfoWithSound", "Rien d'anormal à signaler !"] spawn life_fnc_showPredefinedNotification;
};