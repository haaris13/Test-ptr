...
if ((life_vehicle_to_custom isKindOf "Air") && (_flagF in ["reb"]) && life_custom_type == "civ") exitWith {
	closeDialog 0;
	["Error", "C'est Impossible pour ce véhicule Ici !"] spawn life_fnc_showPredefinedNotification;
};
...
if (isClass(missionConfigFile >> "CfgVehicleUpgrade" >> life_custom_actualGroup)) then {
