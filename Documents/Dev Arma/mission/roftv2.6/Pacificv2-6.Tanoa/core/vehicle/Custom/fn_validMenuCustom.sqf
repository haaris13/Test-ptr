...
if ((lbCurSel ListUpdgrade) == -1) exitWith {["Error", "Aucune Séléction !"] spawn life_fnc_showPredefinedNotification;};
...
if (life_atmbank < _priceTotal) exitWith {
	closeDialog 0;
	playSound "addItemFailed";
	hint format["Vous n'avez pas l'argent nécésaire sur le compte en banque ! Manque: %1", [(_priceTotal - life_atmbank)] call life_fnc_numberText];
};
...
if (_className == life_custom_classVeh) then {
	...
	if (isClass(missionConfigFile >> "CfgImmatriculationTags" >> (typeOf _vehicle))) then {
		...
		[_vehicle, _imma] call life_fnc_immatriculeVehSet;
		[_vehicle, _imma] remoteExecCall ["life_fnc_immatriculeVehSet",RSERV];
	};
