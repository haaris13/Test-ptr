...
if(!isNil {_vehicle getVariable "mining"}) exitWith {["Error", localize "STR_NOTF_DeviceIsMining"] spawn life_fnc_showPredefinedNotification;}; //Mining is already in process..
closeDialog 0; //Close the interaction menu.
life_action_inUse = true; //Lock out the interaction menu for a bit..
_weight = [_vehicle] call life_fnc_vehicleWeight;
if((_weight select 1) >= (_weight select 0)) exitWith {["Error", localize "STR_NOTF_DeviceFull"] spawn life_fnc_showPredefinedNotification; life_action_inUse = false;};
...
if (_zoneType != "mine") exitWith {["Error", "Cet engin est prévu pour fonctionner uniquement dans une mine pour extraire des ressources !"] spawn life_fnc_showPredefinedNotification; life_action_inUse = false;};
if(_zone == "") exitWith {
	["Error", localize "STR_NOTF_notNearResource"] spawn life_fnc_showPredefinedNotification;
...
if(_item == "") exitWith {["Error", "Ressource invalide ! Ceci est un bug qui en devrait pas avoir lieu..."] spawn life_fnc_showPredefinedNotification; life_action_inUse = false;};
