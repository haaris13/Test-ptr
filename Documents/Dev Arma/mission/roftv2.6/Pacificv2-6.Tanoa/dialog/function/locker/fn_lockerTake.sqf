...
if ((lbCurSel 14002) isEqualTo -1) exitWith {["Error", "Vous n'avez pas sélectionné un objet à prendre"] spawn life_fnc_showPredefinedNotification;};
...
if (_filter isEqualTo 1) then {
	...
	if(_amountToTake > _currentAmount) exitWith {["Error", "Vous ne pouvez en prendre autant"] spawn life_fnc_showPredefinedNotification;};
	...
	if(_amountToTake == 0) exitWith {["Warning", localize "STR_NOTF_InvFull"] spawn life_fnc_showPredefinedNotification;};
	...
	["Accepted", "Vous pouvez en prendre autant"] spawn life_fnc_showPredefinedNotification;
	};
};
