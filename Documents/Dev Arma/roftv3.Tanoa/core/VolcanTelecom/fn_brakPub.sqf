...
if(_robber distance _shop > 5) exitWith { hint "Vous devez être à moins de 5m !" };

if (currentWeapon _robber == "") exitWith { hint "Vous ne pouvez pas braquer sans arme !" };
if (currentWeapon _robber == "Binocular") exitWith { hint "Vous ne pouvez pas braquer sans arme !" };
if (currentWeapon _robber == "Rangefinder") exitWith { hint "Vous ne pouvez pas braquer sans arme !" };

if(vehicle player != player) exitWith {};
//Var checks
if(_shop getVariable ["rob_empty",false]) exitWith {hint "Volcan télécom a été braqué récemment et ne peut pas être braqué à nouveau pendant un certain temps.";};
if(_shop getVariable ["rob_ip",false]) exitWith {hint "Volcan télécom est déjà en train d'étre braqué !";};
...
if(_chance >= 0) then { 	
	hint "Le journaliste a frappé une alarme silencieuse, la police a été alerté !";
...
if(_shop getVariable ["rob_empty",false]) exitWith {
	hint "Anti Dupli ! (Admins Prévenue)";
