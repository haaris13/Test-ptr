...
_hideout = (nearestObjects[getPosATL player,["Flag_White_F"],25]) select 0;
_group = _hideout getVariable ["gangOwner",grpNull];

// Check:
if (west countSide playableUnits < 3) exitWith {
    hint "Il n'y a pas assez de Policier pour effectuer cette action."; };
if ((currentWeapon player) in ["", "Binocular", "Rangefinder"]) exitWith { hint "Vous ne pouvez pas sans arme !" };
if ((!license_civ_insurge) && (!license_civ_mafia) && playerSide == civilian) exitWith {hint "Vous ne faites pas parti d'un gang mafieu ou insurge !";};
...
if (playerSide == west) then {
	...
	_zone = ["zone_",_hideout] joinString "";
