...
if (currentWeapon player == "") exitWith { hint "Vous ne pouvez pas sans arme !" };
if (currentWeapon player == "Binocular") exitWith { hint "Vous ne pouvez pas sans arme !" };
if (currentWeapon player == "Rangefinder") exitWith { hint "Vous ne pouvez pas sans arme !" };

if ((!license_civ_insurge) && (!license_civ_mafia) && playerSide == civilian) exitWith {hint "Vous ne faites pas parti d'un gang mafieu ou insurgé !";};
...
if (playerSide == civilian) then {
	...
	if(_group == grpNull) exitWith {titleText["La zone est déja sécurisé","PLAIN"]};
};
...
if(!isNull _group) then {
	...
	if (playerSide == civilian) then {
		...
		_text = format ["<t color='#ff0000' size = '.8'>Attention, La Police est en train de récupérer la zone de %1 !</t>", _hideout getVariable "zone_name"];
	};
