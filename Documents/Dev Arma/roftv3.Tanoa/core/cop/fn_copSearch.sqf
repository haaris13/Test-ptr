...
if(count _invs > 0) then {
	...
	_informerG = 0;
	{
		...
		if (_informer ==0 && _name in ["archeTOssements","archeTCasque","archeTUniforme","archeSVin","archeSCoffreB","archeSCoffreT"]) then {
			_informer = 1;
			hint "Le civil doit avoir la licence d'Archéologie !";
		};
		...
		if(!isNull (missionConfigFile >> "VirtualItems" >> _name >> "processedItem")) then {
...
//hint format["%1",_this];
hint parseText format["<t color='#FF0000'><t size='2'>%1</t></t><br/><t color='#FFD700'><t size='1.5'><br/>" +(localize "STR_Cop_IllegalItems")+ "</t></t><br/>%2<br/><br/><br/><br/><t color='#FF0000'>%3</t><br/><br/> il avait: %4 d'argent sale "
,(_civ GVAR ["realname",name _civ]),_inv,if(_robber) then {"Robbed the bank"} else {""},_argentsal];
