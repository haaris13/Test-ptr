...
//admin tools new
	...
	case 19: {
		...
		if(_shift && ((ITEM_VALUE("zipties") >= 1)) && playerSide == civilian && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {(cursorTarget GVAR "playerSurrender")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
			[] call life_fnc_restrainAction;
		};
...
// rajouté Nekro
				...
				private _list = ["landVehicle","Air","Ship"];
	...
	case 24: {
		if(_shift) then {
			...
			switch (player getVariable["Earplugs",0]) do {
				case 0: {hintSilent "Ear Plugs 90%"; 0.5 fadeSound 0.1; 0.5 fadeMusic 0.1; player setVariable ["Earplugs", 10]; };
				case 10: {hintSilent "Ear Plugs 60%"; 0.5 fadeSound 0.4; 0.5 fadeMusic 0.1; player setVariable ["Earplugs", 40]; };
				case 40: {hintSilent "Ear Plugs 30%"; 0.5 fadeSound 0.7; 0.5 fadeMusic 0.1; player setVariable ["Earplugs", 70]; };
				case 70: {hintSilent "Ear Plugs retirée"; 0.5 fadeSound 1; 0.5 fadeMusic 0.1; player setVariable ["Earplugs", 0]; };
		    };
		};
	};
