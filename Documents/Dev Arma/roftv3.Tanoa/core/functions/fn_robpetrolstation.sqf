...
_cashStored = 22000 + round(random 40000); //The amount the shop has to rob, you could make this a parameter of the call 
_action = [_this,2] call BIS_fnc_param;//Action name
_name = [_this,3] call BIS_fnc_param;//Shop Name  

if (["thug"] call life_fnc_hasPerk) then {
	_cashStored = _cashStored * 1.1;
};

//Checks
if({side _x == west} count playableUnits < 2 && life_debug == 0 ) exitWith {hint "Il n'y a pas assez de flics en ligne pour faire cela ! (2 minimum)"}; 
if(side _robber != civilian) exitWith {};
if(_robber distance _shop > 5) exitWith { hint "Vous devez être à moins de 5m de la caisse pour voler !" };
if (currentWeapon _robber == "") exitWith { hint "Vous ne pouvez pas voler la station sans arme !" };
if (currentWeapon _robber == "Binocular") exitWith { hint "Vous ne pouvez pas voler la station sans arme !" };
if (currentWeapon _robber == "Rangefinder") exitWith { hint "Vous ne pouvez pas voler la station sans arme !" };

if(vehicle player != player) exitWith {};
//Var checks
if(_shop getVariable ["rob_empty",false]) exitWith {hint "Cette boutique a été volé récemment et ne peut pas être volé à nouveau pendant un certain temps.";};
if(_shop getVariable ["rob_ip",false]) exitWith {hint "Cette boutique est déjà en train d'étre volée !";};
...
if(_chance >= 0) then { 	
	hint "Le caissier a frappé une alarme silencieuse, la police a été alerté !";
...
{			
	...
	if(_cP >= 1) exitWith {};		
	if(_robber distance _shop > 5.5) exitWith {hint "Vous devez rester à moins de 5m ! - Maintenant, les flics sont sur le chemin.";};		
