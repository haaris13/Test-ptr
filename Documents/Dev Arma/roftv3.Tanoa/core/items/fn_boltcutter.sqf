...
_proprio = (_building getVariable "house_owner") select 0;
...
if ((_building getVariable [format ["bis_disabled_Door_%1",_door],0]) isEqualTo 0) exitWith {hint localize "STR_House_Raid_DoorUnlocked"};
    ...
    {
    if (getPlayerUID _x == (_building getVariable "house_owner") select 0) exitWith {
...
switch (typeOf _building) do {
    case "Land_HouseDoubleAL": {_cpRate = 0.003;};
...
life_vehicles pushBack _building;

} else {
  hint "L'outil a cassé";
};
/*
