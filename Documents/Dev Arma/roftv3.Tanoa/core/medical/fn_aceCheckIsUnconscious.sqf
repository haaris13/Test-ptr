...
[] spawn {
	while {true} do {
		...
		if (alive player && !(player getVariable "ACE_isUnconscious")) then {
			{
				if (!(_x in life_isUnconscious_Players) && (_x getVariable "ACE_isUnconscious")) then {
				...
				if (!(_x in life_isUnconscious_Players) && !(alive _x)) then {
					life_isUnconscious_Players pushBack _x;
...
[] spawn {
	while {true} do {
		uiSleep 1;
		{
			if (!(_x getVariable "ACE_isUnconscious") && (alive _x) || !(isPlayer _x) || isNull _x) then {
