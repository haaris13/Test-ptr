...
if(isNull (findDisplay 2900)) then {
	if (_type isEqualTo 1) then {
		if(!createDialog "SettingsMenuLogin") exitWith {hint "Un problème est survenu, le menu ne s'ouvre pas?"};
	} else {
		if(!createDialog "SettingsMenu") exitWith {hint "Un problème est survenu, le menu ne s'ouvre pas?"};
	};
};
