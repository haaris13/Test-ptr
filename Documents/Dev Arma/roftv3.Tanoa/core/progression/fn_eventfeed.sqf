...
_newCtrlPos = [_this,3,[1.11, 0.97, 0.4, 0.1],[[]],4] call BIS_fnc_param;
...
switch (_type) do {
	...
	case "REPUTATIONGAIN":{_text = parseText format["<t align = 'right'>Votre réputation a augmenté de <t color = '#ff3300'>%2</t>",_message, _messageValue]};
	case "REPUTATIONLOSS":{_text = parseText format["<t align = 'right'>Votre réputation a diminué de <t color = '#ff3300'>%2</t>",_message, _messageValue]};
	...
	case "CRAFTED":{_text = parseText format["<t align = 'right'>Vous avez craft %2 %1(s)",_message, _messageValue]};
