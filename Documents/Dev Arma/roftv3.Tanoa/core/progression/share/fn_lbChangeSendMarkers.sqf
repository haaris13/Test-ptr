...
if (_price isEqualTo 0) then {
	...
	if ((_profCurrentLevel - _price) < _profRequiredLevel) then {
		(CONTROL(18100,18102)) ctrlSetStructuredText parseText format ["<t size='1'>Coût(%2): <t color='#b70000'>%1 Level</t></t>", _price, _displayName];
		CONTROL(18100,18103) ctrlEnable false;
	} else {
		(CONTROL(18100,18102)) ctrlSetStructuredText parseText format ["<t size='1'>Coût(%2): <t color='#b2ec00'>%1 Level</t></t>", _price, _displayName];
		CONTROL(18100,18103) ctrlEnable true;
	};
};