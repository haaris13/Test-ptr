...
switch (playerSide) do { 
	case civilian : { _currentProfiles = ((profileNamespace getVariable ["radio_profiles_civ", str []]));}; 
	case west : { _currentProfiles = (profileNamespace getVariable ["radio_profiles_west", str []]); }; 
	case independent : { _currentProfiles = (profileNamespace getVariable ["radio_profiles_med", str []]); }; 
};

if (typeName _currentProfiles == typeName "") then {
	_currentProfiles = call (compile _currentProfiles);
} else {
	switch (playerSide) do {
