...
if (_vehicle getVariable["trunk_cache_use", false]) exitWith {hint "Quelqu'un est déja en train de gérer la contrebande !";};
...
_sound = false;
if (["fasthide"]call life_fnc_hasPerk) then {
	_cp = 1;
};
...
if(!isNil "_badDistance") exitWith {titleText["Vous étes trop loin !","PLAIN"]; life_action_inUse = false; _vehicle setVariable["trunk_cache_use", false, true];};
