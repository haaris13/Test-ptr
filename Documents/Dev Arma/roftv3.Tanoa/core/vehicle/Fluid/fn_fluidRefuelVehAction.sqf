...
if(!([str(_editText)] call life_fnc_isnumeric)) exitWith {hint localize "STR_ATM_notnumeric"};
...
if ((_citerne select 0) == "" || (_citerne select 1) == 0) exitWith {hint "Véhicule Vide !"; playSound "addItemFailed";};
if ((_citerne select 1) < _editText) exitWith {hint "Pas assez dans le réservoir !"; playSound "addItemFailed";};
...
switch (_vehArray select 1) do { 
	case "Cargo" : {
		...
		if ((_curValue select 0) != "" && (_curValue select 0) != (_citerne select 0)) exitWith {
			hint "Un fluide est déjà présent dans ce véhicule !";
