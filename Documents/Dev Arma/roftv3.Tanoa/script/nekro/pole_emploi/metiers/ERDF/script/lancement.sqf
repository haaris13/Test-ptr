...
{
	hint "Tu as déjà un métier dépêches-toi d y aller ou je te fous un coup de pied au cul !";
	closedialog 0;
};

if (missionERDF != 0) exitWith
{
	hint format ["Désolé, Tu est déjà en mission REDSG fini la rapidement !"];
	closedialog 0;
};
...
{ 
	...
	diag_log format ["Mise à jour de la variable global nekro_ERDF_maxplayer a  %1",nekro_ERDF_maxplayer]; 
	publicVariable "nekro_ERDF_maxplayer";
};

/* CONDITION MAX PLAYERS  */
if (nekro_ERDF_maxplayer >= _maxplyers) exitWith
{
	nekro_ERDF_maxplayer = 0;
	publicVariable "nekro_ERDF_maxplayer";
	[(name player)] remoteExecCall ["life_fnc_VerificationMetiers"];
	hint format ["Désolé, nous ne recherchons plus de personnel pour ce metier actuellement. Il y a déjà assez de monde !"];
	closedialog 0;
	JobDisable = 1;
	uisleep 60;
	JobDisable = 0;
...
/* SET MARKER */
_markERDF = createMarkerLocal [format["Patron_SAUR"],getpos ERDFStart];
...
waitUntil {position player distance ERDFStart < 5};
